...
** Amministrazione
Nel caso di login di un utente con privilegi di amministratore, il menu risulta leggermente modificato, in primis, compare la voce *Users*, che mostra
tutti gli utenti registrati nella base di dati e offre la possibilità di modificarne i dati, eliminare profili o aggiungerne.
Le voci *Collections*, *Playlists* mostrano tutte le collezioni e playlist create, con relativi dati dei creatori, anche qui è possibile cancellare, 
creare e modificare tali elementi.\\
L'amministratore infine ha pieno potere di creazione e cancellazione / modifica dei brani e album.
...
la variabile ~uid~ si occupa di tenere traccia dell'id dell'utente autenticato. La variabile ~admin~ infine, viene creata per tenere traccia dei privilegi
dell'utente, un valore 1 rappresenta i massimi privilegi amministrativi, 0 per utenti ordinari.\\
